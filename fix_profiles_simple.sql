-- Simple script to fix missing profiles for existing users
...
-- 5. Create profiles for users who don't have them (with skills array)
...
SELECT 
  ...
  CASE 
    WHEN u.raw_user_meta_data->>'skills' IS NOT NULL AND u.raw_user_meta_data->>'skills' != '[]'
    THEN string_to_array(trim(both '[]' from u.raw_user_meta_data->>'skills'), ',')
    ELSE ARRAY[]::TEXT[]
  END,
...
-- 7. Check the final result - all professionals
SELECT 
  p.id,
  p.user_id,
  p.full_name,
